...
# Step 1: Pick a repo.
repo_line="$("$TAISM" repos | fzf --prompt "repo> " --reverse --no-multi --bind "tab:down,shift-tab:up")"
[[ -z "$repo_line" ]] && exit 0
repo_path="$(echo "$repo_line" | awk -F'\t' '{print $2}')"
repo_name="$(echo "$repo_line" | awk -F'\t' '{print $1}')"

# Step 2: Name the branch (freehand input via --print-query with empty list).
branch="$(echo "" | fzf --prompt "branch name> " --print-query --no-info 2>/dev/null | head -1)"
[[ -z "$branch" ]] && exit 0
...
if [[ "$agent_count" -le 1 ]]; then
  ...
  agent="$(echo "$agent_list" | fzf --prompt "agent> " --reverse)"
  [[ -z "$agent" ]] && exit 0
fi
